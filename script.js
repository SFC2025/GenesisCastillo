// ==================== Config rápida ====================
const WA_NUMBER = "0000000000"; // Solo dígitos, ej: 5493511234567
...
(function initPayPalButtons(retries = 25) {
  ...
  const map = {
    id: "TV2PUYLEXR4DS",
    container: "#paypal-container-TV2PUYLEXR4DS",
  };
  const el = document.querySelector(map.container);
  if (!el) return; // <-- nada que renderizar aca

  const ready = !!(window.paypalHB && paypalHB.HostedButtons);
  if (ready) {
    paypalHB
      .HostedButtons({ hostedButtonId: map.id })
      .render(map.container)
      .catch((err) =>
        console.error(`PayPal HB render ${map.container} falló:`, err)
      );
    return;
  }

  if (retries > 0) {
    setTimeout(() => initPayPalButtons(retries - 1), 200);
  } else {
    console.warn("PayPal HB no disponible (SDK no cargado en esta página).");
  }
})();

// ==================== PayPal Subscription Button ====================
(function renderSubscription(retries = 25) {
  ...
  paypalSUB
    .Buttons({
      ...
      createSubscription: (data, actions) =>
        actions.subscription.create({ plan_id: "P-6SV06465S5864740NNDAHZ6Y" }),
      onApprove: (data) => {
        location.href =
